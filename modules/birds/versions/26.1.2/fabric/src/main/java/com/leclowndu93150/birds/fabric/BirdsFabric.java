...
public class BirdsFabric implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        ResourceKey<Item> itemKey = ResourceKey.create(Registries.ITEM, Birds.id("bird_spawn_egg"));
        Item birdItem = new SpawnEggItem(new Item.Properties().setId(itemKey).spawnEgg(birdType));
        Registry.register(BuiltInRegistries.ITEM, itemKey.identifier(), birdItem);
        Birds.BIRD_ITEM = birdItem;

        BiomeModifications.addSpawn(BiomeSelectors.tag(SPAWNS_BIRDS), MobCategory.AMBIENT, birdType, 50, 7, 10);
        SpawnPlacements.register(birdType, SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING, Bird::checkBirdSpawnRule);

        CreativeModeTabEvents.modifyOutputEvent(CreativeModeTabs.SPAWN_EGGS).register(content -> {
            content.accept(birdItem);
        });

        CreativeModeTab birdsTab = FabricCreativeModeTab.builder()
