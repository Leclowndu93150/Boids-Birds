...
public class BirdsNeoForge {
    ...
    public static final DeferredHolder<EntityType<?>, EntityType<Bird>> BIRD = ENTITY_TYPES.register("bird",
        () -> EntityType.Builder.of(Bird::new, MobCategory.AMBIENT).sized(1.5f, 0.6f).clientTrackingRange(16).build(Birds.id("bird").toString()));

    public static final DeferredHolder<Item, Item> BIRD_SPAWN_EGG = ITEMS.register("bird_spawn_egg",
        () -> new SpawnEggItem(BIRD.get(), 0xFF4D3927, 0xFF7D706C, new Item.Properties()));
    ...
    private void onSpawnPlacementRegister(RegisterSpawnPlacementsEvent event) {
        event.register(BIRD.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING, Bird::checkAnimalSpawnRules, RegisterSpawnPlacementsEvent.Operation.OR);
    }
